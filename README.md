...
Each saved run can be exported into a local, shareable review package with **Export Run Results**. The default package excludes raw video, uses that run's own saved input snapshot rather than today's live site files, and includes a README explaining what is inside. Raw video is only ever added when the **Include raw video only if this video is approved to share** checkbox is checked, and only for videos whose current file still matches the checksum saved for that run. See [Run export packages](docs/architecture/run-export-packages.md) for the package layout.
