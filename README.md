...
Each saved run can be exported with **Export Run Results**, and every site can be exported at once with **Export All Reports** (next to Refresh). Both download a zip through the browser. The single-run export is a portable copy of that run's own folder: a teammate can copy it straight into their own `data/sites/<site>/outputs/runs/` and their Home UI picks it up, no import step needed. Export All bundles every site's config, labels, manifest, and run history the same way, so a teammate without that site set up locally can copy a whole site folder in and start reviewing. Both use each run's own saved input snapshot rather than today's live site files. Raw video is excluded by default in both; the single-run export only adds it when the **Include raw video only if this video is approved to share** checkbox is checked and the file still matches the checksum saved for that run, and Export All only adds it when its own raw-video checkbox is checked. See [Run export packages](docs/architecture/run-export-packages.md) for the layout.
